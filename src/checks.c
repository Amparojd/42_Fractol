...
int	arg_check(char *argv_1, char *argv_2, char *argv_3)
{
	...
	{
		if (ft_strtodouble(argv_2) && ft_strtodouble(argv_3)
			&& ft_strtodouble(argv_2) >= -2.0 && ft_strtodouble(argv_2) <= 2.0
			&& ft_strtodouble(argv_3) >= -2.0 && ft_strtodouble(argv_3) <= 2.0)
			return (1);
	}
...
{
	f->name = fractal_type;
	f->julia_r = ft_strtodouble(j_r);
	f->julia_i = ft_strtodouble(j_i);
	julia(f);
}
